...
use crate::model_info::API_PROVIDER_ID;
...
pub struct CursorApiAuthStatus {
    pub connected: bool,
    pub key_preview: Option<String>,
    pub last_validated_ms: Option<i64>,
}

impl CursorApiAuthStatus {
...
impl ApiCredential {
    ...
    pub fn from_sinew_auth_file(path: &Path) -> Result<Option<Self>> {
        ...
        if payload.provider != API_PROVIDER_ID || payload.auth_mode != "api_key" {
            return Ok(None);
        }
        let api_key = payload.tokens.api_key.trim();
        if api_key.is_empty() {
            return Err(AppError::Auth("cursor api auth is missing API key".into()));
        }
...
pub fn save_default_api_key(api_key: &str) -> Result<CursorApiAuthStatus> {
    let api_key = api_key.trim();
    if api_key.is_empty() {
        return Err(AppError::Auth("Cursor API key cannot be empty".into()));
    }
    let auth = StoredAuth {
        provider: API_PROVIDER_ID.into(),
...
pub fn load_default_api_auth_status() -> Result<CursorApiAuthStatus> {
    load_api_auth_status(&default_auth_path()?)
}

pub fn load_api_auth_status(path: &Path) -> Result<CursorApiAuthStatus> {
    let bytes = match std::fs::read(path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return Ok(CursorApiAuthStatus::disconnected())
        }
        Err(err) => return Err(AppError::Auth(format!("unable to read auth file: {err}"))),
    };
    let payload: StoredAuth = serde_json::from_slice(&bytes)
...
pub fn delete_default_api_auth() -> Result<()> {
...
pub fn default_auth_path() -> Result<PathBuf> {
    ...
    Ok(dirs.data_local_dir().join("cursor-api-auth.json"))
}

fn status_from_auth(auth: &StoredAuth) -> CursorApiAuthStatus {
    if auth.provider != API_PROVIDER_ID || auth.auth_mode != "api_key" {
        return CursorApiAuthStatus::disconnected();
    }
