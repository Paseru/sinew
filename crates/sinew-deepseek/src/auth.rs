...
impl Credential {
    ...
    pub fn from_sinew_auth_file(path: &Path) -> Result<Option<Self>> {
        ...
        if api_key.is_empty() {
            return Err(AppError::Auth("deepseek auth is missing API key".into()));
        }
...
pub fn save_default_api_key(api_key: &str) -> Result<DeepSeekAuthStatus> {
...
pub fn touch_default_auth_validation() -> Result<DeepSeekAuthStatus> {
    let path = default_auth_path()?;
    let bytes = match std::fs::read(&path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return Ok(DeepSeekAuthStatus::disconnected())
        }
        Err(err) => return Err(AppError::Auth(format!("unable to read auth file: {err}"))),
    };
...
pub fn load_default_auth_status() -> Result<DeepSeekAuthStatus> {
    load_auth_status(&default_auth_path()?)
}

pub fn load_auth_status(path: &Path) -> Result<DeepSeekAuthStatus> {
    let bytes = match std::fs::read(path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return Ok(DeepSeekAuthStatus::disconnected())
...
fn default_auth_path() -> Result<PathBuf> {
    ...
    Ok(dirs.data_local_dir().join("deepseek-auth.json"))
}

fn status_from_auth(auth: &StoredAuth) -> DeepSeekAuthStatus {
    if auth.provider != PROVIDER_ID || auth.auth_mode != "api_key" {
        return DeepSeekAuthStatus::disconnected();
    }
