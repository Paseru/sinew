                ...
                match source.next().await {
                    Some(Ok(event)) => {
                        ...
                        match parsed {
                            ...
                            Err(err) => {
                                return Some((
                                    Err(AppError::Decode(format!("bad ollama event: {err}"))),
                                    (source, parser, pending, true, saw_any_event),
                                ));
                            }
                        }
                    ...
                    None => {
                        if !saw_any_event {
                            return Some((
                                Err(AppError::Stream(
                                    "ollama SSE closed before any event; \
            ...
            if let Some(state) = self.tool_states.remove(&key) {
                if let Some(index) = state.part_index {
                    out.push(StreamEvent::PartMeta {
                        index,
                        meta: json!({ "provider": "ollama", "id": state.id, "name": state.name }),
                    });
                    out.push(StreamEvent::PartStop { index });
                }
            }
    ...
    chunk
        ...
        .or_else(|| {
            chunk
                ...
                .then(|| "ollama stream error".to_string())
        })
}

fn reasoning_delta(delta: &wire::ChatDelta) -> Option<String> {
    if let Some(value) = delta.thinking.as_deref().filter(|value| !value.is_empty()) {
