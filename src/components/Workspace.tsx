...
export function Workspace({
  ...
  // ---------------- SSH Widget State & Handlers ----------------
  const [sshHost, setSshHost] = useState<string | null>(null);
  const [sshWidgetOpen, setSshWidgetOpen] = useState(false);
  ...
  useEffect(() => {
    try {
      const host = localStorage.getItem(`ssh_mount_${bootstrap.workspace.path}`);
      setSshHost(host);
    } catch (e) {
      setSshHost(null);
    }
  ...
  useEffect(() => {
    if (sshWidgetOpen) {
      api.listSshHosts().then(setSshHosts).catch(() => {});
    }
  }, [sshWidgetOpen]);
  ...
  const handleConnectSsh = async (target: string) => {
    if (streamingConversationIds.size > 0) {
      setSshError("Une conversation est en cours. Veuillez l'arrêter avant de vous connecter.");
      return;
    }
    setConnectingSsh(true);
    setSshError(null);
    try {
      const b = await api.mountSshWorkspace(target);
      try {
        localStorage.setItem(`ssh_mount_${b.workspace.path}`, target);
      } catch (e) {}
      
      const displayName = b.workspace.name === ".sinew-sandbox" ? "Sans dossier" : b.workspace.name;
      recordRecent(b.workspace.path, displayName);
    ...
    <div className="workspace">
      ...
      <div className="main">
        ...
        >
          ...
          <div className="sidebar-ssh-widget">
            {!sshWidgetOpen ? (
              <div className="sidebar-ssh-widget__collapsed">
                <div className="sidebar-ssh-widget__status">
                  <span
                    className={`sidebar-ssh-widget__status-dot ${
                      sshHost
                        ? "sidebar-ssh-widget__status-dot--connected"
                        : "sidebar-ssh-widget__status-dot--disconnected"
                    }`}
                  />
                  {sshHost ? (
                    <span title={`Connecté en SSH à ${sshHost}`}>
                      SSH: {sshHost}
                    </span>
                  ) : (
                    <span>SSH non connecté</span>
                  )}
                </div>
                {sshHost ? (
                  <button
                    type="button"
                    className="sidebar-ssh-widget__btn"
                    title="Déconnecter"
                    onClick={handleDisconnectSsh}
                  >
                    <Icon icon="solar:logout-linear" width={14} height={14} />
                  </button>
                ) : (
                  <button
                    type="button"
                    className="sidebar-ssh-widget__connect-btn"
                    onClick={() => setSshWidgetOpen(true)}
                  >
                    Se connecter
                  </button>
                )}
              ...
              <div className="sidebar-ssh-widget__form">
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: "11px", fontWeight: "600", color: "var(--text-1)" }}>Connexion SSH</span>
                  <button
                    type="button"
                    className="sidebar-ssh-widget__btn"
                    onClick={() => {
                ...
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    if (sshTarget.trim()) {
                      void handleConnectSsh(sshTarget.trim());
                    }
                  }}
                  className="sidebar-ssh-widget__form-row"
                >
                  <input
                    type="text"
                    className="sidebar-ssh-widget__input"
                    placeholder="IP, domaine ou alias"
                    value={sshTarget}
                  ...
                  <button
                    type="submit"
                    className="sidebar-ssh-widget__submit-btn"
                ...
                {sshError && (
                  <div className="sidebar-ssh-widget__error">
                    {sshError}
                  </div>
                )}
                {sshHosts.length > 0 && (
                  <div className="sidebar-ssh-widget__hosts">
                    <div className="sidebar-ssh-widget__hosts-title">Favoris (SSH config)</div>
                    {sshHosts.map((host) => (
                      <button
                        key={host}
                        type="button"
                        className="sidebar-ssh-widget__host-item"
                        disabled={connectingSsh}
                        onClick={() => void handleConnectSsh(host)}
                      >
                        <span>{host}</span>
                        <Icon icon="solar:alt-arrow-right-bold" width={10} height={10} />
                      </button>
                    ))}
                  </div>
                )}
