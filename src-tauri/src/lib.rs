...
use sinew_app::{
    ...
    copy_workspace_entries, codebase_index_status, create_installed_skill, create_workspace_directory,
    ...
    AppStore, BashTool, CheckSotaTool, CodebaseSearchTool, ComputerUseTool, ConversationEvent, ConversationSummary, CreateImageTool,
    DeleteFileTool, EditFileTool, GlobTool, GoalWorkflowState, GrepTool,
...
pub(crate) fn run_git_auto_pull(workspace_path: &str) {
    #[cfg(target_os = "windows")]
    {
        ...
        tracing::info!("Starting robust Git Auto-Pull for workspace: {:?}", clean);

        // 1. If there are uncommitted changes, commit them first to avoid pull blockages
        if has_uncommitted_changes(&clean) {
        ...
        match status {
            Ok(s) if s.success() => {
                tracing::info!("Git Auto-Pull and rebase succeeded!");
            }
            _ => {
...
pub(crate) fn run_git_auto_commit_and_push(workspace_path: &str) {
    #[cfg(target_os = "windows")]
    {
        ...
        tracing::info!("Starting robust Git Auto-Commit and Push for workspace: {:?}", clean);
        ...
        match pull_status {
            Ok(s) if s.success() => {
                ...
                match push_status {
                    Ok(ps) if ps.success() => {
                        tracing::info!("Git Auto-Push on exit succeeded!");
                    }
                    other => {
                        tracing::error!("Git Auto-Push on exit failed: {:?}", other);
                    }
                }
            }
            _ => {
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            workspace::mount_super_ssh_workspace,
            ...
            workspace::codebase_index_stats_command,
            ...
            providers::archive_provider,
            providers::restore_provider,
            providers::list_archived_providers,
