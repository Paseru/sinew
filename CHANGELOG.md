...
### Added
- **Synchronisation automatique des jetons de connexion (OAuth/cl�s) via OneDrive (`src-tauri/src/lib.rs`)** : Ajout de la copie automatique des fichiers de connexion (*-auth.json, *-device.json, *-stream-state.json) lors du d�marrage, de la fermeture et de la synchronisation forc�e. Cela permet de conserver la connexion aux comptes d'IA (Claude, ChatGPT, etc.) identique sur les deux ordinateurs sans devoir se r�authentifier manuellement.
